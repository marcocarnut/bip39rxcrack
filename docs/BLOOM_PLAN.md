...
1c. **DONE** — missing-word bloom (`g_crack_nth_bloom`/`g_crack_missing_bloom`; on a
    culled-true hit we re-derive that one index single-target to recover the words for
    rendering); `--addresses` folded into the **work-queue** (both `--words` and `--template`
    fan out over all GPUs; workers build their own filter); the **compact bloom path**
    (`g_sieve_perm` → `g_pbkdf2_perm_bloom`) so `--words --addresses` runs at the ~20 Mc/s
